...
folder = "../data/back up data/03_24_2019"

coord_names = sorted(glob.glob(folder + "/coordinates/*.txt"))
image_names = sorted(glob.glob(folder + "/images/*.png"))

##imgcoord_names = sorted(glob.glob("../data/back up data/03_24_2019/image_coords/*.txt"))
...
noise = generate_noise(size,rad,0,0.2)*255

single_particles = np.zeros((size,(rad*2)**2))
##for image in noise:
...
np.savetxt("noise particles_0324.txt", noise, fmt="%d", delimiter=' ')
np.savetxt("particle images_0324.txt", single_particles, fmt="%d", delimiter=' ')

print(np.loadtxt("noise particles_0324.txt").shape)
